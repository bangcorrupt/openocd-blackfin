...
int jtag_libusb_open_with_options(const uint16_t vids[], const uint16_t pids[],
		const char *serial,
		struct jtag_libusb_device_handle **out,
		int num_options, ...)
{
	...
	while(num_options > 0) {
		option = va_arg(ap, enum libusb_option);
		switch (option) {
		case LIBUSB_OPTION_LOG_LEVEL:
			arg = va_arg(ap, int);
			if (libusb_set_option(jtag_libusb_context, option, arg) != LIBUSB_SUCCESS)
				return ERROR_FAIL;
			break;

		case LIBUSB_OPTION_USE_USBDK:
			if (libusb_set_option(jtag_libusb_context, option) != LIBUSB_SUCCESS)
				return ERROR_FAIL;
			break;
